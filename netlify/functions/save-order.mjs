  ...
  try {
    ...
    const order = {
      ...
      data: {
        ...
        length: clean(body.length,100),
        fullness: clean(body.fullness,100),
        primaryColor: clean(body.primaryColor,40),
        primaryColorName: clean(body.primaryColorName,100),
        secondaryColor: clean(body.secondaryColor,40),
        secondaryColorName: clean(body.secondaryColorName,100),
        accentColor: clean(body.accentColor,100),
        style: clean(body.style,100),
        studentName,
        nickname: clean(body.nickname,150),
        school: clean(body.school,200),
        mascot: clean(body.mascot,150),
        grade: clean(body.grade,80),
        gradYear: clean(body.gradYear,40),
        number: clean(body.number,40),
        homecomingDate: clean(body.homecomingDate,40),
        activities: clean(body.activities),
        favorites: clean(body.favorites),
        song: clean(body.song,500),
        quote: clean(body.quote,1000),
        specialDate: clean(body.specialDate,500),
        addons: Array.isArray(body.addons) ? body.addons.slice(0,30).map(x=>clean(x,200)) : [],
        braid: clean(body.braid,200),
        printedRibbon: clean(body.printedRibbon,200),
        ribbonText: clean(body.ribbonText,1000),
        instructions: clean(body.instructions,8000),
    ...
    if (!index.includes(orderNumber)) {
      index.unshift(orderNumber);
      index = index.slice(0,1000);
      await store.setJSON("order-index", index);
    }
